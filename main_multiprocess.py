...
def get_sftp(sftp_icdx_folder_path, sftp_icpreg_folder_path):
            ...
            with pysftp.Connection(host='IP', username='username', password='pwd', cnopts=cnopts, port='port') as sftp:
                ...
                for filename in sftp.listdir():
                    if 'ICDX_' in filename:
                        ...
                        try:
                            copyfile(os.path.abspath(os.path.join(sftp_icdx_folder_path, filename)),
                                     os.path.abspath(os.path.join('/home/rserver/task/COVID_DEATH_SURVEILLANCE_LINE_NOTIFY/', 'sftp_download_folder', 'ICDX_sftp', filename)))
                    ...
                    if 'ICPREG_' in filename:
                        ...
                        try:
                            copyfile(os.path.abspath(os.path.join(sftp_icpreg_folder_path, filename)),
                                     os.path.abspath(os.path.join('/home/rserver/task/COVID_DEATH_SURVEILLANCE_LINE_NOTIFY/', 'sftp_download_folder', 'ICPREG_sftp', filename)))
                        except Exception as e:
                            print(e)
                        print(filename)
    ...
    if sftp_filename_list[0] and sftp_filename_list[1]:
        with open(os.path.join('/home/rserver/task/VACCINE_AESI/', 'sftp_download.pickle'), 'wb') as pl:
...
def data_divide_func(sftp_filename_list, division_name):
    ...
    for f_n in sftp_filename_list:
        ...
        chunk = 10 ** 7
        _df_ = pd.read_csv(os.path.join('.', 'sftp_download_folder', f'{division_name}_sftp', f_n), header=[0], low_memory=False, chunksize=chunk)
        ...
        for df_ in _df_:
            ...
            n = 0
            for i in range(0, len(df_), 5000000):
                n += 5000000
                if n < len(df_):
    ...
    if df_list:
        if len(df_list) <= 1:
            with open(os.path.join('.', 'daily_pickle_store_folder', f'{division_name}_pickle', f"df_final_{division_name}_{datetime.datetime.today().strftime('%Y%m%d')}.pickle"), 'wb') as pl:
                df_dump = df_list[0].drop_duplicates().reset_index(drop=True)
                pickle.dump(df_dump, pl)
...
def file_list_select(division_name, s_date):
    file_list = []
    s_date = datetime.datetime.strptime(str(s_date), '%Y%m%d').date()
    for file in os.listdir(os.path.join('.', 'sftp_download_folder', f'{division_name}_sftp')):
        date = file.split('.txt')[0].split(f'{division_name}_')[1]
...
def main():
    ...
    test_icdx_filename_list = [
        'ICDX_20210411.txt', 'ICDX_20210412.txt', 'ICDX_20210413.txt', 'ICDX_20210414.txt', 'ICDX_20210415.txt',
        'ICDX_20210416.txt', 'ICDX_20210417.txt', 'ICDX_20210418.txt', 'ICDX_20210419.txt', 'ICDX_20210420.txt', 'ICDX_20210421.txt',
        'ICDX_20210422.txt', 'ICDX_20210423.txt', 'ICDX_20210424.txt', 'ICDX_20210425.txt', 'ICDX_20210426.txt', 'ICDX_20210427.txt',
        'ICDX_20210428.txt', 'ICDX_20210429.txt', 'ICDX_20210430.txt', 'ICDX_20210501.txt', 'ICDX_20210502.txt', 'ICDX_20210503.txt',
    ...
    test_icpreg_filename_list = ['ICPREG_20210411.txt', 'ICPREG_20210412.txt', 'ICPREG_20210413.txt', 'ICPREG_20210414.txt',
                                 'ICPREG_20210415.txt', 'ICPREG_20210416.txt', 'ICPREG_20210417.txt', 'ICPREG_20210418.txt', 'ICPREG_20210419.txt',
                                 'ICPREG_20210420.txt', 'ICPREG_20210421.txt', 'ICPREG_20210422.txt', 'ICPREG_20210423.txt', 'ICPREG_20210424.txt',
                                 'ICPREG_20210425.txt', 'ICPREG_20210426.txt', 'ICPREG_20210427.txt', 'ICPREG_20210428.txt', 'ICPREG_20210429.txt',
                                 'ICPREG_20210430.txt', 'ICPREG_20210501.txt', 'ICPREG_20210502.txt', 'ICPREG_20210503.txt', 'ICPREG_20210504.txt',
                                 ...
                                 'ICPREG_20210515.txt', 'ICPREG_20210516.txt'
                                 ]

    for d in ['ICDX', 'ICPREG']:
        filename_list = file_list_select(d, 20210410)
        data_divide_func(filename_list, d)
